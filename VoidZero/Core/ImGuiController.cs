...
namespace VoidZero.Core
{
    public class ImGuiController : IDisposable
    {
        ...
        public void LoadFont(string fontPath, float fontSize, out ImFontPtr fontPtr)
        {
            // Finish the current frame if active
            if (_frameBegun)
            {
                ImGui.Render();
                _frameBegun = false;
            ...
            // Add the font
            fontPtr = io.Fonts.AddFontFromFileTTF(fontPath, fontSize);

            // Build the atlas and update GPU texture
            io.Fonts.Build();
            RecreateFontDeviceTexture();
        }
    }
